...
# open the datasets using xarray and convert to dataframes (coarsen the model to regional boxes?)
print('opening met inputs...')
met_dset_orig = monetio.models.cmaq.open_mfdataset(mei_fnames)
#met_dset=met_dset_orig
#met_dset=met_dset_orig.coarsen(x=3,boundary="trim").mean().coarsen(y=3,boundary="trim").mean()
met_dset=met_dset_orig.coarsen(x=6,boundary="trim").max().coarsen(y=6,boundary="trim").mean()
...
met_df.query('latitude > @latmin & latitude < @latmax & longitude < @lonmax & longitude > @lonmin',inplace=True)

print('opening ant. emission inputs...')
aei_dset_orig = monetio.models.cmaq.open_mfdataset(aei_fnames)
#aei_dset=aei_dset_orig
#aei_dset=aei_dset_orig.coarsen(x=3,boundary="trim").mean().coarsen(y=3,boundary="trim").mean()
aei_dset=aei_dset_orig.coarsen(x=6,boundary="trim").max().coarsen(y=6,boundary="trim").mean()

aei_df=aei_dset[['NH3','SO2','PEC','POC','NO','NO2','VOC_INV','CO']].to_dataframe().reset_index()
aei_df.rename(columns={'NH3': 'AE_NH3', 'SO2': 'AE_SO2', 'PEC': 'AE_PEC', 'POC': 'AE_POC', 'NO': 'AE_NO', 'NO2': 'AE_NO2', 'VOC_INV': 'AE_VOC', 'CO': 'AE_CO'}, inplace=True)
...
#bei_dset=bei_dset_orig.coarsen(x=3,boundary="trim").mean().coarsen(y=3,boundary="trim").mean()
bei_dset=bei_dset_orig.coarsen(x=6,boundary="trim").max().coarsen(y=6,boundary="trim").mean()
...
#fei_dset=fei_dset_orig.coarsen(x=3,boundary="trim").mean().coarsen(y=3,boundary="trim").mean()
fei_dset=fei_dset_orig.coarsen(x=6,boundary="trim").max().coarsen(y=6,boundary="trim").mean()

fei_df   = fei_dset[['NH3','SO2', 'AECI', 'AECJ', 'ASO4I', 'ASO4J', 'ANO3I', 'ANO3J', 'ANH4I', 'ANH4J', 'NO','NO2','CO','ISOP','OLE','PAR','MEOH','TERP','ETH','ETOH','ACET','ALDX','IOLE','FORM','ALD2','ETHA','KET']].to_dataframe().reset_index()
fei_df.rename(columns={'NH3': 'FE_NH3', 'SO2': 'FE_SO2', 'AECI': 'FE_AECI', 'AECJ': 'FE_AECJ',  'ASO4I': 'FE_ASO4I',  'ASO4J': 'FE_ASO4J',  'ANO3I': 'FE_ANO3I', 'ANO3J': 'FE_ANO3J','ANH4I': 'FE_ANH4I', 'ANH4J': 'FE_ANH4J', 'NO': 'FE_NO', 'NO2': 'FE_NO2', 'CO': 'FE_CO','ISOP': 'FE_ISOP', 'OLE': 'FE_OLE', 'PAR': 'FE_PAR', 'MEOH': 'FE_MEOH','TERP': 'FE_TERP','ETH': 'FE_ETH','ETOH': 'FE_ETOH','ACET': 'FE_ACET','ALDX': 'FE_ALDX','IOLE': 'FE_IOLE','FORM': 'FE_FORM','ALD2': 'FE_ALD2','ETHA': 'FE_ETHA','KET': 'FE_KET'}, inplace=True)
...
fei_df['FE_AEC']=fei_df['FE_AECI']+fei_df['FE_AECJ']
fei_df['FE_ASO4']=fei_df['FE_ASO4I']+fei_df['FE_ASO4J']
fei_df['FE_ANO3']=fei_df['FE_ANO3I']+fei_df['FE_ANO3J']
fei_df['FE_ANH4']=fei_df['FE_ANH4I']+fei_df['FE_ANH4J']
fei_df.drop(columns=['x', 'y', 'z','FE_ISOP','FE_OLE','FE_PAR','FE_MEOH','FE_TERP','FE_ETH','FE_ETOH','FE_ACET','FE_ALDX','FE_IOLE','FE_FORM','FE_ALD2','FE_ETHA','FE_KET','FE_NO','FE_NO2','FE_AECI','FE_AECJ','FE_ASO4I','FE_ASO4J','FE_ANO3I','FE_ANO3J','FE_ANH4I','FE_ANH4J'], inplace=True)
...
#ozo_dset=ozo_dset_orig.coarsen(x=3,boundary="trim").max().coarsen(y=3,boundary="trim").max()
ozo_dset=ozo_dset_orig.coarsen(x=6,boundary="trim").mean().coarsen(y=6,boundary="trim").max()

ozo_df=ozo_dset['PM25_TOT'].to_dataframe().reset_index()
ozo_df.query('latitude > @latmin & latitude < @latmax & longitude < @lonmax & longitude > @lonmin',inplace=True)

print('calculate 24hr rolling pm25 average...')
ozo_df['PM25_TOT_24hr']=ozo_df['PM25_TOT'].rolling(24).mean()
#ozo_df['target']=np.where(ozo_df['O3']>= 70.0, 1, 0)
ozo_df['target']=np.where(ozo_df['PM25_TOT_24hr']>= 35.0, 1, 0)
ozo_df.drop(columns=['x', 'y', 'z','PM25_TOT','PM25_TOT_24hr'], inplace=True)
...
plt.savefig("Figure_1_shap_value_coarse_pm25_tot_metonly_"+region+".png", format='png', dpi='figure', bbox_inches='tight')
plt.close()

## Generate summary bar plot
shap.summary_plot(shap_values, X,plot_type="bar",show=False)
plt.savefig("Figure_2_mean_shap_value_coarse_pm25_tot_metonly_"+region+".png", format='png', dpi='figure', bbox_inches='tight')
plt.close()

## Generate specific dependence plots
shap.dependence_plot("PBL", shap_values, X, interaction_index="TEMP2", alpha=0.1, show=False)
plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_metonly_"+region+"_PBL_TEMP2.png", format='png', dpi='figure', bbox_inches='tight')
plt.close()

shap.dependence_plot("TEMP2", shap_values, X, interaction_index="PBL", alpha=0.1, show=False)
plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_metonly_"+region+"_TEMP2_PBL.png", format='png', dpi='figure', bbox_inches='tight')
plt.close()

shap.dependence_plot("GSW", shap_values, X, interaction_index="CFRAC", alpha=0.1, show=False)
plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_metonly_"+region+"_GSW_CFRAC.png", format='png', dpi='figure', bbox_inches='tight')
plt.close()

shap.dependence_plot("CFRAC", shap_values, X, interaction_index="GSW", alpha=0.1, show=False)
plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_metonly_"+region+"_CFRAC_GSW.png", format='png', dpi='figure', bbox_inches='tight')
plt.close()

shap.dependence_plot("Q2", shap_values, X, interaction_index="PRECIP", alpha=0.1, show=False)
plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_metonly_"+region+"_Q2_PRECIP.png", format='png', dpi='figure', bbox_inches='tight')
plt.close()

#shap.dependence_plot("BE_VOC", shap_values, X, interaction_index="TEMP2", alpha=0.1, show=False)
#plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_"+region+"_BE_VOC_TEMP2.png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()
#
#shap.dependence_plot("TEMP2", shap_values, X, interaction_index="BE_VOC", alpha=0.1, show=False)
#plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_"+region+"_TEMP2_BE_VOC.png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()
#
#shap.dependence_plot("BE_VOC", shap_values, X, interaction_index="GSW", alpha=0.1, show=False)
#plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_"+region+"_BE_VOC_GSW.png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()
#
#shap.dependence_plot("BE_VOC", shap_values, X, interaction_index="LAI", alpha=0.1, show=False)
#plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_"+region+"_BE_VOC_LAI.png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()
#
#shap.dependence_plot("LAI", shap_values, X, interaction_index="BE_VOC", alpha=0.1, show=False)
#plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_"+region+"_LAI_BE_VOC.png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()
#
#shap.dependence_plot("GSW", shap_values, X, interaction_index="BE_VOC", alpha=0.1, show=False)
#plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_"+region+"_GSW_BE_VOC.png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()
#
#shap.dependence_plot("BE_NO", shap_values, X, interaction_index="PRECIP", alpha=0.1, show=False)
#plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_"+region+"_BE_NO_PRECIP.png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()
#
#shap.dependence_plot("PRECIP", shap_values, X, interaction_index="BE_NO", alpha=0.1, show=False)
#plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_"+region+"_PRECIP_BE_NO.png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()

shap.dependence_plot("WDIR10", shap_values, X, interaction_index="WSPD10", alpha=0.1, show=False)
plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_metonly_"+region+"_WDIR10_WSPD10.png", format='png', dpi='figure', bbox_inches='tight')
plt.close()

shap.dependence_plot("WDIR10", shap_values, X, interaction_index="TEMP2", alpha=0.1, show=False)
plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_metonly_"+region+"_WDIR10_TEMP2.png", format='png', dpi='figure', bbox_inches='tight')
plt.close()

shap.dependence_plot("WSPD10", shap_values, X, interaction_index="WDIR10", alpha=0.1, show=False)
plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_metonly_"+region+"_WSPD10_WDIR10.png", format='png', dpi='figure', bbox_inches='tight')
plt.close()

shap.dependence_plot("AE_NOX", shap_values, X, interaction_index="AE_VOC", alpha=0.1, show=False)
plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_"+region+"_AE_NOX_AE_VOC.png", format='png', dpi='figure', bbox_inches='tight')
plt.close()

#shap.dependence_plot("AE_VOC", shap_values, X, interaction_index="AE_NOX", alpha=0.1, show=False)
#plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_"+region+"_AE_VOC_AE_NOX.png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()

#shap.dependence_plot("AE_CO", shap_values, X, interaction_index=None, alpha=0.1, show=False)
#plt.savefig("Figure_3_dependence_plot_coarse_pm25_tot_"+region+"_AE_CO.png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()

### Generate multiple dependence plots
#for name in X_train.columns:
#     shap.dependence_plot(name, shap_values, X, show=False)
#     plt.savefig("Figure_4_dependence_plot_coarse_"+region+"_"+name+".png", format='png', dpi='figure', bbox_inches='tight')
#     plt.close()
#
...
plt.savefig("Figure_5_waterfall_legacy_first_coarse_pm25_tot_metonly_"+region+".png", format='png', dpi='figure', bbox_inches='tight')
plt.close()
#
...
plt.savefig("Figure_5_waterfall_legacy_last_coarse_pm25_tot_metonly_"+region+".png", format='png', dpi='figure', bbox_inches='tight')
plt.close()
#
## Generate force plot - Single row
#shap.force_plot(explainer.expected_value, shap_values[0], feature_names=X.columns, show=False)
#plt.savefig("Figure_6_force_single_first_coarse_"+region+".png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()
#
#shap.force_plot(explainer.expected_value, shap_values[last_explanation], feature_names=X.columns, show=False)
#plt.savefig("Figure_6_force_single_last_coarse_"+region+".png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()
#
...
#plt.savefig("Figure_6_force_multiple_100_coarse_"+region+".png", format='png', dpi='figure', bbox_inches='tight')
#plt.close()
#
...
plt.savefig("Figure_7_decision_plot_first_coarse_pm25_tot_metonly_"+region+".png", format='png', dpi='figure', bbox_inches='tight')
plt.close()
#
...
plt.savefig("Figure_7_decision_plot_last_coarse_pm25_tot_metonly_"+region+".png", format='png', dpi='figure', bbox_inches='tight')
plt.close()
#
...
plt.savefig("Figure_7_decision_plot_multiple_100_coarse_pm25_tot_metonly_"+region+".png", format='png', dpi='figure', bbox_inches='tight')
plt.close()
